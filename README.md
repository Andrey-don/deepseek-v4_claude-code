...
```
VS Code
├── Терминал 1: claude        → Claude Sonnet 4.6 (оркестратор)
├── Терминал 2: claude-ds     → DeepSeek V4 Pro (исполнитель, ~5x дешевле)
└── Терминал 3: deepseek (опционально) → DeepSeek TUI
```

Claude Sonnet 4.6 выступает тимлидом и делегирует простые задачи DeepSeek через bash-команду.
...
Экономия: **4-5x** на простых и средних задачах.
